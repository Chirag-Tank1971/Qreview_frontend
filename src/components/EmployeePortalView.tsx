  ...
  if (!isLoading && (!currentEmp || employees.length === 0)) {
    return (
      <div className="space-y-6">
        ...
        <EmptyState
          icon={Users}
          title="No Employee Profiles in Database"
          description={
          ...
          action={{ label: 'Go to Bulk Data Tools', icon: Upload, onClick: () => { window.location.hash = '#bulk'; } }}
          secondaryAction={{ label: 'Employee Directory', icon: UserCheck, onClick: () => { window.location.hash = '#employees'; } }}
        />
      </div>
    );
  }
