...
export const EmployeePortalView: React.FC<EmployeePortalViewProps> = ({
  ...
  const handleAcknowledge = async (appraisalId: string) => {
    if (!ackAccepted) {
      alert('Please check the acknowledgement confirmation box before submitting.');
      return;
    }
    try {
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        {(user?.role === 'SUPER_ADMIN' || user?.role === 'HR') && (
          <div className="flex items-center gap-2.5 bg-slate-50 p-2 rounded-xl border border-slate-200 self-start md:self-auto">
            <label className="text-xs font-semibold text-slate-600 shrink-0 flex items-center gap-1.5">
            ...
            <select
              ...
              className="text-xs font-semibold text-slate-800 bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer shadow-2xs"
            >
              {employees.map((emp) => (
      ...
      {isLoading ? (
        <div className="p-12 text-center bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-xs font-semibold text-slate-600">Loading Employee Self-Service Dashboard...</p>
        </div>
      ) : error ? (
        ...
        <>
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
            <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center font-bold text-2xl border border-slate-200 shrink-0">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-3 pt-1 text-[11px] text-slate-500">
                    <span className="flex items-center gap-1">
                      ...
                      <span>Manager: <strong>{currentEmp.managerName || 'Rohan Deshmukh'}</strong></span>
                    </span>
                    <span className="flex items-center gap-1">
                      <Building2 className="w-3.5 h-3.5 text-slate-400" />
                      <span>HOD: <strong>{currentEmp.hodName || 'Vikram Mehta'}</strong></span>
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 self-stretch lg:self-auto justify-start lg:justify-end">
                {/* 8-Cycle Badge */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center min-w-[130px]">
                ...
                {/* Current CTC Badge */}
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center min-w-[140px]">
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">
                    Current Fixed CTC
                  </span>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-center min-w-[120px]">
                  ...
                  <div className="text-base font-extrabold text-amber-600 font-mono mt-0.5 flex items-center justify-center gap-1">
          ...
          {/* Action Items / Notifications Alert if pending */}
          {essData.actionItems && essData.actionItems.length > 0 && (
            <div className="space-y-2">
              {essData.actionItems.map((item: any) => (
                <div
                  key={item.id}
                  className={`p-4 rounded-xl border flex items-center justify-between gap-3 text-xs shadow-2xs ${
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {item.type === 'APPRAISAL_ACKNOWLEDGE_DUE' ? (
                  ...
                  <button
                    onClick={() => {
                      if (item.linkTab === 'appraisal') {
                        setActiveTab('appraisal');
                      } else if (item.linkTab === 'reviews' && item.reviewId) {
                        const targetRev = reviews.find((r) => r.id === item.reviewId);
                        if (targetRev) setSelectedReviewForSelfAssess(targetRev);
                        setActiveTab('reviews');
                      }
                    }}
                    className={`px-3 py-1.5 rounded-lg font-bold text-xs shrink-0 cursor-pointer shadow-2xs flex items-center gap-1 ${
                      item.priority === 'URGENT'
          ...
          {/* Navigation Tabs */}
          <div className="flex items-center space-x-1 border-b border-slate-200 bg-white p-1 rounded-xl shadow-2xs">
            <button
            ...
            >
              ...
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded-md bg-slate-100 text-slate-700">
                {reviews.length}
              </span>
            </button>
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1">
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-slate-500">STAGE 1</span>
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                        </div>
                        <div className="text-xs font-bold text-slate-800">Manager Recommendation</div>
                      ...
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-slate-500">STAGE 2</span>
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                        </div>
                        <div className="text-xs font-bold text-slate-800">HOD Department Calibration</div>
                        <div className="text-[10px] text-slate-500">Budget Cleared & Approved</div>
                      </div>

                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] font-bold text-slate-500">STAGE 3</span>
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                        </div>
                        <div className="text-xs font-bold text-slate-800">HR Final Approval</div>
                        <div className="text-[10px] text-slate-500">Letter Formatted & Verified</div>
                      </div>
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
                    ...
                    {activeAppraisal.promotionRecommended && (
                      <div className="p-4 bg-amber-50 border-b border-amber-200 flex items-center gap-3 text-amber-900">
                    ...
                    <div className="p-5">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-emerald-50/80 border border-emerald-200 rounded-xl space-y-1">
                          ...
                          <div className="text-lg font-bold text-emerald-700 font-mono flex items-center gap-1.5">
                        ...
                        {isAcknowledged ? (
                          <div className="p-5 bg-emerald-50 border border-emerald-200 rounded-2xl space-y-3">
                            ...
                            {activeAppraisal.employeeAcknowledgement?.comments && (
                              <div className="p-3 bg-white/70 rounded-xl border border-emerald-200 text-xs text-emerald-900 italic">
                                "{activeAppraisal.employeeAcknowledgement.comments}"
                              </div>
                            )}
                          ...
                          <div className="p-5 bg-white border-2 border-indigo-200 rounded-2xl space-y-4 shadow-sm">
                            <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-3 pt-2">
                              <label className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 border border-slate-200 cursor-pointer hover:bg-slate-100/70 transition-colors">
                                <input
                                  ...
                                  className="mt-0.5 w-4 h-4 rounded text-slate-600 focus:ring-indigo-500 border-slate-300"
                                />
                              ...
                              <div className="flex items-center justify-between pt-2">
                                <button
                                  type="button"
                                  onClick={() => setSelectedAppraisalForLetter(activeAppraisal)}
                                  className="text-xs text-slate-600 hover:text-indigo-800 font-semibold flex items-center gap-1 cursor-pointer"
                                >
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5">
                            <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">Basic Salary (50%)</span>
                          ...
                          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5">
                            <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">House Rent (HRA 25%)</span>
                            <div className="text-sm font-bold text-slate-900 font-mono mt-1">
                          ...
                          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5">
                            <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">Special Allowance (20%)</span>
                            <div className="text-sm font-bold text-slate-900 font-mono mt-1">
                          ...
                          <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5">
                            <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider block">Provident Fund (5%)</span>
                            <div className="text-sm font-bold text-slate-900 font-mono mt-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {reviews.map((rev) => {
                      ...
                      return (
                        <div
                          key={rev.id}
                          className="bg-slate-50/70 hover:bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-5 transition-all space-y-4 shadow-2xs"
                        >
                          {/* Quarter Header */}
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-lg font-bold font-mono text-slate-900">
                                {rev.finalScore ? (
                                  <span>{rev.finalScore.toFixed(2)} <span className="text-xs text-slate-400">/ 5.0</span></span>
                                ) : (
                          ...
                          {rev.strengths && (
                            <div className="text-xs text-slate-600 bg-white p-2.5 rounded-lg border border-slate-100 space-y-0.5">
                          ...
                          <div className="pt-1 flex items-center justify-between border-t border-slate-200">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-slate-900 hover:bg-slate-800 text-white transition-all shadow-2xs cursor-pointer"
                            >
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            {kra.description && (
                              <p className="text-xs text-slate-500 mt-0.5">{kra.description}</p>
                            )}
                          </div>
                        </div>

                        <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-slate-100 text-slate-800 border border-slate-200 font-mono self-start">
                          Weight: {kra.weight}%
                        </span>
                      </div>
                      ...
                      <div className="bg-white p-3 rounded-lg border border-slate-200 text-xs space-y-1">
                        <div className="flex items-start gap-1.5">
                          <span className="font-semibold text-slate-700 shrink-0">Target Metric:</span>
                          <span className="text-slate-600">{kra.target || kra.targetSnapshot || 'Sprint deliverables on time'}</span>
                        </div>
                        {kra.measurementCriteria && (
                          <div className="flex items-start gap-1.5 pt-0.5">
                            <span className="font-semibold text-slate-700 shrink-0">Evaluation Rubric:</span>
                            <span className="text-slate-500 text-[11px]">{kra.measurementCriteria}</span>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div className="space-y-6">
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {reviews.map((r, i) => (
                      <div key={i} className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-center space-y-2">
                        <span className="text-[11px] font-bold text-slate-500 block">{r.reviewPeriodName}</span>
