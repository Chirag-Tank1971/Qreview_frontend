  ...
  const handleSubmit = async (isDraft: boolean) => {
    try {
      ...
      if (!isDraft) {
        ...
        if (unratedKra) {
          setError(`Please provide a self-rating for "${unratedKra.kraName || unratedKra.title}".`);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden my-auto">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          {/* Quick Score Banner */}
          <div className="bg-gradient-to-r from-slate-900 to-indigo-950 rounded-2xl p-4 sm:p-5 text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
            ...
            {review.finalScore && (
              <div className="p-3 bg-white/10 rounded-xl border border-white/10 text-center sm:text-right">
          ...
          <div className="space-y-4">
            ...
            {kraStates.map((kra, index) => {
              ...
              return (
                <div
                  key={kra.id || index}
                  className="p-4 sm:p-5 rounded-xl border border-slate-200 bg-white hover:border-slate-300 transition-all space-y-3.5 shadow-2xs"
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                    <div className="space-y-1">
                      ...
                      {kra.description && (
                        <p className="text-xs text-slate-500 pl-7">{kra.description}</p>
                      )}
                    </div>

                    <div className="flex items-center gap-2 self-start pl-7 sm:pl-0">
                      <span className="text-[11px] font-bold px-2.5 py-1 rounded-md bg-slate-100 text-slate-800 border border-slate-200 font-mono">
                        Weight: {kra.weight}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs space-y-1">
                    <div className="flex items-start gap-1.5">
                      ...
                      <span className="text-slate-600">{kra.targetSnapshot}</span>
                    </div>
                    {kra.measurementCriteria && (
                      <div className="flex items-start gap-1.5 pt-0.5">
                        <span className="font-semibold text-slate-700 shrink-0">Rubric:</span>
                  ...
                  <div className="space-y-2 pt-1">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-700 flex items-center gap-1">
                        ...
                        <span className="text-indigo-600 font-mono">{kra.selfRating} / 5</span>
                      </label>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full text-xs p-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-slate-50/50"
                    />
                  </div>
                  ...
                  {kra.rating !== undefined && (
                    <div className="p-2.5 bg-indigo-50/60 border border-indigo-100 rounded-lg text-xs space-y-1">
                      ...
                      {kra.comments && (
                        <p className="text-[11px] text-indigo-800 italic">"{kra.comments}"</p>
                      )}
                    </div>
                  )}
                </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/80 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center space-x-2 w-full sm:w-auto justify-end">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-100 rounded-xl border border-slate-200 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            ...
            {!isClosed && (
              <>
                <button
                  ...
                  className="flex items-center space-x-1.5 px-4 py-2 text-xs font-semibold text-slate-700 bg-white hover:bg-slate-100 rounded-xl border border-slate-300 transition-colors cursor-pointer shadow-2xs"
                >
                ...
                <button
                  ...
                  className="flex items-center space-x-1.5 px-5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-xs cursor-pointer"
                >
